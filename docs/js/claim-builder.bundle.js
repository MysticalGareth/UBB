/* UBB Claim Builder - Browser Bundle */
"use strict";
var UBBClaimBuilder = (() => {
  ...
  // src/browser/claim-builder.ts
  var claim_builder_exports = {};
  __export(claim_builder_exports, {
    buildClaimOpReturnHex: () => buildClaimOpReturnHex
  });
  init_buffer_shim();

  // src/transactions/build-claim-opreturn.ts
  init_buffer_shim();
  ...
  // src/ubb-plot.ts
  init_buffer_shim();
  var UBBPlot = class _UBBPlot {
    constructor(bmp, x0, y0) {
      this.CANVAS_SIZE = 65536;
      this._bmp = bmp;
      this._x0 = x0;
      this._y0 = y0;
     ...
     */
    atPosition(x0, y0) {
      return new _UBBPlot(this._bmp, x0, y0);
    }
    _validatePlacement() {
      const errors = [];
      if (!this.isWithinCanvas(this._x0, this._y0, this._bmp.width, this._bmp.height)) {
        errors.push(
          `Plot extends outside canvas bounds. Position: (${this._x0}, ${this._y0}), Size: ${this._bmp.width}x${this._bmp.height}, Canvas: ${this.CANVAS_SIZE}x${this.CANVAS_SIZE}`
        );
      }
      ...
      if (this._x0 + this._bmp.width > this.CANVAS_SIZE || this._y0 + this._bmp.height > this.CANVAS_SIZE) {
        errors.push(
          `Plot extends beyond canvas boundaries. Right edge: ${this._x0 + this._bmp.width}, Bottom edge: ${this._y0 + this._bmp.height}, Canvas limit: ${this.CANVAS_SIZE}`
        );
      }
      return {
        errors: Object.freeze([...errors]),
    ...
    isWithinCanvas(x0, y0, width, height) {
      return x0 >= 0 && y0 >= 0 && x0 + width <= this.CANVAS_SIZE && y0 + height <= this.CANVAS_SIZE && width > 0 && height > 0;
    }
  };

  // src/transactions/build-claim-opreturn.ts
  function buildClaimOpReturnHex(x, y, uriOrBmpHex, maybeBmpHex) {
    const uri = maybeBmpHex !== void 0 ? uriOrBmpHex : "";
    const bmpHex = maybeBmpHex !== void 0 ? maybeBmpHex : uriOrBmpHex;
    const errors = {};
    ...
    if (bmpHexErrors.length > 0) {
      ...
      try {
        bmpBuffer = import_buffer.Buffer.from(bmpHex, "hex");
  ...
  function validateCoordinate(coord, name) {
    const errors = [];
  ...
  function validateBmpHex(bmpHex) {
    const errors = [];
  ...
  function buildPayload(x, y, uri, bmpBuffer) {
    const magic = import_buffer.Buffer.from([19, 55]);
    const version = import_buffer.Buffer.from([1]);
    const txType = import_buffer.Buffer.from([1]);
    const xBuffer = import_buffer.Buffer.allocUnsafe(2);
    xBuffer.writeUInt16LE(x, 0);
    const yBuffer = import_buffer.Buffer.allocUnsafe(2);
    yBuffer.writeUInt16LE(y, 0);
    const uriBuffer = encode(uri);
  ...
  return __toCommonJS(claim_builder_exports);
})();
...
buffer/index.js:
   ...
   *)
*/
//# sourceMappingURL=claim-builder.bundle.js.map
